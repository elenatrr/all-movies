...
const theMovieDbApi = axios.create({
    ...
    headers: {
        Authorization: `Bearer ${import.meta.env.VUE_APP_THE_MOVIE_DB_API_KEY}`,
        Accept: 'application/json'
    }
});
const getMoviesData = async (category, page = 1) => {
    try {
...
const getMovieDetailsData = async (id) => {
    try {
...
const getRecommendedMoviesData = async (id) => {
    try {
